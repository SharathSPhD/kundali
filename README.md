...
Validation: 128 tests, including golden charts (Einstein/Vivekananda/Nehru/Bose from AA-rated birth data; Raman-ayanamsa degree-level checks), a Drik Panchang position capture matched to <2 arcmin, B.V. Raman's Standard Horoscope Shadbala pinned against PyJHora, and Amitabh Bachchan's Chara Dasha reproducing K.N. Rao's published sequence. Future: bulk validation against the open vedastro 15k-chart dataset (HuggingFace).
- `frontend/` — Next.js 14 (App Router, TS, Tailwind). Supabase auth, birth profiles + life events, South Indian SVG chart, varga selector, dasha timeline, prediction cards with substantiation, grounded chat. Runs without Supabase in local mode (localStorage).
- `supabase/schema.sql` — tables + row-level security.
- `ARCHITECTURE.md` — design decisions and API contract. `DEPLOYMENT.md` — hosting steps.
...
```bash
cd frontend
rm -rf node_modules   # clear partial install left by the build sandbox
npm install
npm run dev           # http://localhost:3000 (local mode, no Supabase needed)
```
Tests: `cd backend && pytest` (55 passing; 3 golden-case tests skip until `tests/fixtures/case_study.json` has real birth data).
