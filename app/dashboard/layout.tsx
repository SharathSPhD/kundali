...
export default function DashboardLayout({
  ...
  const navLinks: Array<{ href: string; label: string; icon: typeof Moon }> = [
    { href: "/dashboard", label: "Profiles", icon: Moon },
  ...
  return (
    <div className="min-h-screen">
      ...
      <header className="border-b border-night-600/50 bg-night-900/60 backdrop-blur">
        ...
        {/* Mobile nav drawer */}
        {menuOpen && (
          <nav className="animate-fade-up flex flex-col gap-1 border-t border-night-600/50 px-4 py-3 text-sm sm:hidden">
            {navLinks.map(({ href, label, icon: Icon }) => (
              <Link
                key={href}
                href={href}
